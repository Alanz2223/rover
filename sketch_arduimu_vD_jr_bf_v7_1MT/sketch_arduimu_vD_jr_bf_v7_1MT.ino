// JR -- see docs https://code.google.com/p/ardu-imu/downloads/list
...
#define Accel_Scale(x) x*(GRAVITY/9.81)//Scaling the raw data of the accel to actual acceleration in meters for seconds square
#define Accel_Scale_fps_jr(x) ((x)*(32.17/(GRAVITY)))//Scaling the raw data of the accel to actual acceleration in metersconds square
...
struct FilterInfo filters[12] ;

float ua = 0.0 ;
float ub = 0.0 ;
float northVelocity = 0 ;
float eastVelocity = 0 ; 
float northPosition = 0 ;
float eastPosition = 0 ;

 float Ndotdot  = 0.0 ;
 float Edotdot  = 0.0 ;
 
 // Integrate in abs frame to get velocities    
 float Ndot = 0.0 ;
 float Edot = 0.0 ;
...
volatile uint8_t analog_count[8];
 ...
 // JR
 void calculateFilterCoefficients ( struct FilterInfo* filt, float p0, float p1,
  float q0, float q1, float T, char* name )
{
        filt->b0 = p0 * T + 2.0 * p1 ;
        filt->a0 = ( q0 * T + 2.0 * q1) / filt->b0 ;     // a0
        filt->a1 = ( q0 * T - 2.0 * q1) / filt->b0 ;     // a1
        filt->b1 = ( p0 * T - 2.0 * p1) / filt->b0;     // b1
        //strcpy ( filt->id, name ) ;
}

void initFilterStateVars ( struct FilterInfo* filt, float ul, float yl )
{
  filt->ul = ul ;
  filt->yl = yl ;
}

// Returns filtered value, updates ul and yl in caller
float filter ( float u, struct FilterInfo* fi )
{
     // This is the filter equation
    float y = fi->a0 * u + fi->a1 * fi->ul - fi->b1 * fi->yl ;
    //float y = A0 * u + A1 * (*ul) - B1 * (*yl) ;
    ...
    Serial.print ( "," ) ;
    Serial.print ( "b1=" ) ;
    Serial.println ( fi->b1 ) ;
    */
    // end debug
    
    // Update the state variables outside this function
    fi->ul = u ;
    fi->yl = y ;
...
float mapXY ( float rawIn, float m )
{
  return rawIn * m ;
}
...
// JR
// X channel First integrator
  calculateFilterCoefficients ( &filters[FILT_A], p0, p1, q0, q1, T, "A" ) ;
  initFilterStateVars ( &filters[FILT_A], 0, 0 ) ;

  // Y channel integrator
  calculateFilterCoefficients ( &filters[FILT_B], p0, p1, q0, q1, T, "B" ) ;
  initFilterStateVars ( &filters[FILT_B], 0, 0 ) ;

  // North axis calculation
  calculateFilterCoefficients ( &filters[FILT_C], p0, p1, q0, q1, T, "C" ) ;
  initFilterStateVars ( &filters[FILT_C], 0, 0 ) ;

  // East axis calculation
  calculateFilterCoefficients ( &filters[FILT_D], p0, p1, q0, q1, T, "D" ) ;
  initFilterStateVars ( &filters[FILT_D], 0, 0 ) ;  
  
  // Added new integrators for Lou's method
   calculateFilterCoefficients ( &filters[FILT_E], p0, p1, q0, q1, T, "E" ) ;
  initFilterStateVars ( &filters[FILT_E], 0, 0 ) ;

  // Y channel integrator
  calculateFilterCoefficients ( &filters[FILT_F], p0, p1, q0, q1, T, "F" ) ;
  initFilterStateVars ( &filters[FILT_F], 0, 0 ) ;

  // North axis calculation
  calculateFilterCoefficients ( &filters[FILT_G], p0, p1, q0, q1, T, "G" ) ;
  initFilterStateVars ( &filters[FILT_G], 0, 0 ) ;

  // East axis calculation
  calculateFilterCoefficients ( &filters[FILT_H], p0, p1, q0, q1, T, "H" ) ;
  initFilterStateVars ( &filters[FILT_H], 0, 0 ) ;  
  
  // Init test filters
  calculateFilterCoefficients ( &filters[FILT_I], p0, p1, q0, q1, T, "I" ) ;
  initFilterStateVars ( &filters[FILT_I], 0, 0 ) ; 
  
  calculateFilterCoefficients ( &filters[FILT_J], p0, p1, q0, q1, T, "J" ) ;
  initFilterStateVars ( &filters[FILT_J], 0, 0 ) ; 
  
  calculateFilterCoefficients ( &filters[FILT_K], p0, p1, q0, q1, T, "K" ) ;
  initFilterStateVars ( &filters[FILT_K], 0, 0 ) ; 
  
  calculateFilterCoefficients ( &filters[FILT_L], p0, p1, q0, q1, T, "L" ) ;
  initFilterStateVars ( &filters[FILT_L], 0, 0 ) ; 
...
//
  ...
  // jr and bf code to calculate accel bias
  //int j ;
  for ( int j = 0 ; j < 100 ; j++ )
  {
    Accel_Vector[0] += read_adc(3) ;
    Accel_Vector[1] += read_adc(4) ;
    delay ( 20 ) ;
  }
  accelBias[0] = Accel_Vector[0] / 100 ;
  accelBias[1] = Accel_Vector[1] / 100 ;
  Accel_Vector[0] = 0.0 ;
  Accel_Vector[0] = 0.0 ;
...
void loop() //Main Loop
{
  ...
  if((timeNow-timer)>=20)  // Main loop runs at 50Hz
  {
    ...
    // Barry and Jerry mods, doing 1st integration here
    // Velocity components, U and V, in the body axis
    
    //Accel_Vector[0] -= accelBias[0] ;
    //Accel_Vector[1] -= accelBias[1] ;
    /*
    float U = filter ( Accel_Vector[0], &filters[FILT_A] ) ;
    float V = filter ( Accel_Vector[1], &filters[FILT_B] ) ;
    
     // Now multiply by appropriate elements of DCM to get 
     // NED inertial framework values
     northVelocity = DCM_Matrix[0][0] * U + DCM_Matrix[0][1] * V ;
     eastVelocity = DCM_Matrix[1][0] * U + DCM_Matrix[1][1] * V ;
     
     northPosition = filter ( northVelocity, &filters[FILT_C] ) ;
     eastPosition = filter ( eastVelocity, &filters[FILT_D] ) ;
    // end bf and jr
    */
    /****************************************** v2 (Lou stuff)******************************************/
    ...
    //float V = filter ( Accel_Vector[1], &filters[FILT_B] ) ;
     ...
     // Calculate accels in absolute frame, north and east dirs
     // NED inertial framework values
    
     Ndotdot = DCM_Matrix[0][0] * Accel_Scale_fps_jr((Accel_Vector[0] - accelBias[0])) 
     + DCM_Matrix[0][1] * Accel_Scale_fps_jr((Accel_Vector[1] - accelBias[1])) ;

     Edotdot = DCM_Matrix[1][0] * Accel_Scale_fps_jr((Accel_Vector[0] - accelBias[0])) 
     + DCM_Matrix[1][1] * Accel_Scale_fps_jr((Accel_Vector[1] - accelBias[1])) ;
     
     // Integrate in abs frame to get velocities    
     Ndot = filter ( Ndotdot, &filters[FILT_E] ) ;
     Edot = filter ( Edotdot, &filters[FILT_F] ) ;
     
     // Integrate in abs frame to get positions    
     N = filter ( Ndot, &filters[FILT_G] ) ;
     E = filter ( Edot, &filters[FILT_H] ) ;
    // end bf and jr
   
   /*********************************** end baseline ****************************/
   /*********************************** start test code A ***************************/
   // Adjust
     float AV0 = mapXY ( Accel_Vector[0] - accelBias[0], 0.40 ) ;
     float AV1 = mapXY ( Accel_Vector[1] - accelBias[1], 0.40 ) ;
   
    NdotdotA = DCM_Matrix[0][0] * Accel_Scale_fps_jr((AV0)) 
     + DCM_Matrix[0][1] * Accel_Scale_fps_jr((AV1)) ;

     EdotdotA = DCM_Matrix[1][0] * Accel_Scale_fps_jr((AV0)) 
     + DCM_Matrix[1][1] * Accel_Scale_fps_jr(AV1) ;
     
     // Integrate in abs frame to get velocities    
     NdotA = filter ( NdotdotA, &filters[FILT_A] ) ;
     EdotA = filter ( EdotdotA, &filters[FILT_B] ) ;
     
     // Integrate in abs frame to get positions    
     NA = filter ( NdotA, &filters[FILT_C] ) ;
     EA = filter ( EdotA, &filters[FILT_D] ) ;
     
 
 /************************************ start test code B ****************************/
    // Adjust
     float BV0 = mapXY ( Accel_Vector[0] - accelBias[0], 0.30 ) ;
     float BV1 = mapXY ( Accel_Vector[1] - accelBias[1], 0.30 ) ;
    
    
    NdotdotB = DCM_Matrix[0][0] * Accel_Scale_fps_jr((BV0)) 
     + DCM_Matrix[0][1] * Accel_Scale_fps_jr(BV1) ;

     EdotdotB = DCM_Matrix[1][0] * Accel_Scale_fps_jr((BV0)) 
     + DCM_Matrix[1][1] * Accel_Scale_fps_jr((BV1)) ;
     
     // Integrate in abs frame to get velocities    
     NdotB = filter ( NdotdotB, &filters[FILT_I] ) ;
     EdotB = filter ( EdotdotB, &filters[FILT_J] ) ;
     
     // Integrate in abs frame to get positions    
     NB = filter ( NdotB, &filters[FILT_K] ) ;
     EB = filter ( EdotB, &filters[FILT_L] ) ;
